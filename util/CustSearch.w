...
DEFINE INPUT  PARAMETER xcomp LIKE propsl.comp#.
DEFINE INPUT  PARAMETER xdiv  LIKE propsl.div#.
...
&Scoped-define FIELDS-IN-QUERY-BrowseSearch BigSearch.PROPSL# PROPSL.C-NAME PROPSL.L-NAME BigSearch.Val BigSearch.Fld   
&Scoped-define ENABLED-FIELDS-IN-QUERY-BrowseSearch   
&Scoped-define SELF-NAME BrowseSearch
&Scoped-define QUERY-STRING-BrowseSearch FOR EACH BigSearch WHERE    BigSearch.comp# = xcomp AND    BigSearch.div#  = xdiv    NO-LOCK, ~
             EACH PROPSL WHERE BigSearch.PROPSL# = propsl.propsl# NO-LOCK, ~
             EACH PRO-DESP OF PROPSL NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BrowseSearch OPEN QUERY {&SELF-NAME} FOR EACH BigSearch WHERE    BigSearch.comp# = xcomp AND    BigSearch.div#  = xdiv    NO-LOCK, ~
             EACH PROPSL WHERE BigSearch.PROPSL# = propsl.propsl# NO-LOCK, ~
             EACH PRO-DESP OF PROPSL NO-LOCK INDEXED-REPOSITION.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BrowseSearch ttyDialog _FREEFORM
...
/* Query rebuild information for BROWSE BrowseSearch
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH BigSearch WHERE
   BigSearch.comp# = xcomp AND
   BigSearch.div#  = xdiv
   NO-LOCK,
      EACH PROPSL WHERE BigSearch.PROPSL# = propsl.propsl# NO-LOCK,
      EACH PRO-DESP OF PROPSL NO-LOCK INDEXED-REPOSITION.
...
DO:
 ...
 /*apply lastkey.*/
 /*ASSIGN FillSearch.*/
 /*message fillsearch:SCREEN-VALUE view-as alert-box.*/
 /*OPEN QUERY BrowseSearch
   FOR EACH BigSearch WHERE
      BigSearch.comp# = xcomp AND
      BigSearch.div#  = xdiv  AND
      BigSearch.Val begins FillSearch /*:SCREEN-VALUE IN FRAME {&FRAME-NAME}*/,
       ...
       .*/
  /*message bigsearch.propsl# view-as alert-box. */    
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FillSearch ttyDialog
ON LEAVE OF FillSearch IN FRAME ttyDialog /* Search */
DO:
 ASSIGN FillSearch.
 OPEN QUERY BrowseSearch
   FOR EACH BigSearch WHERE
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FillSearch ttyDialog
ON VALUE-CHANGED OF FillSearch IN FRAME ttyDialog /* Search */
DO:
 OPEN QUERY BrowseSearch
   FOR EACH BigSearch WHERE
      BigSearch.comp# = xcomp AND
